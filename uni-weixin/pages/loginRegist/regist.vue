...
<template>
	<view class="page">
		...
		<view class="navigation-bar" style="z-index: 77777;;background-color: #FFFFFF">
			<view class="nav-left">
				<image v-if="hasLastLoginUser" src="../../static/icons/close.png" class="back-icon" @click="goBack"></image>
			</view>
...
<!-- @input="typingVerifyCode" -->
		...
		<view class="bottom-side-box">
			...
			<view class="change-page-box" v-if="!hasLastLoginUser">
				<text class="txt-bottom-nav" @click="goToNewAccountLogin">已有账号，前往登录</text>
			</view>
	...
	export default {
		data() {
			return {
				...
				loginTouched: false,
				codeTouched: false,
				hasLastLoginUser: false,
			}
		},
		onLoad() {
			this.statusBarHeight = system.statusBarHeight;
			this.hasLastLoginUser = this.hasSavedLoginUser();
		},
		methods: {
			hasSavedLoginUser() {
				var userInfo = app.getUserInfoSession();
				return userInfo != null && userInfo != undefined
					&& userInfo.mobile != null && userInfo.mobile != undefined && userInfo.mobile != "";
			},
			...
			doRegist() {
				...
				uni.request({
					method: "POST",
					url: serverUrl + "/passport/registry",
					data: {
						"mobile": mobile,
						"smsCode": verifyCode,
						"nickname": nickname,
					...
					success(result) {
						...
						if (status == 200) {
							...
							app.setUserSessionTokenKey(userInfo.tokenKey);
							// 登录成功，关闭当前页
							me.goto();
						} else if (status != 200) {
							uni.showToast({
