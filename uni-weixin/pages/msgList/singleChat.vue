...
<template>
	<view>
		...
		<view class="navigation-bar">
			...
			<view class="middle-title">{{friendInfo.nickname}}</view>
		...
		<view class="msg-list-box" :style="{width: '100%', background: 'url(' + myInfo.chatBg + ') no-repeat', backgroundSize:'100% 100%', backgroundAttachment: 'fixed'}">
		<!-- <view class="msg-list-box" style="width: 100%">	 -->
			...
			<scroll-view 
				...
				<view id="msgViewList" class="msg-view-list">
					<view class="line-wrapper" style=""><view class="line" style="margin-top: 10px;width: 100%;height: 1px;opacity: 0;"></view></view>
							...
							<view class="every-msg-right">
								...
								<view class="msg-box-right">
									...
									<view 
										...
										<text class="seconds-right" :style="{'width': ((m.speakVoiceDuration/speakMaxSeconds)*speakMaxWidth+36) + 'rpx'}">{{m.speakVoiceDuration}}″</text>
					...
					<view id="lastLine" class="line-wrapper" style=""><view class="line" style="margin-bottom: 20px;width: 100%;height: 1px;opacity: 0;"></view></view>
				</view>
				
				<view id="lastLineBottom" class="line-wrapper" style=""><view class="line" style="margin-bottom: 20px;width: 100%;height: 1px;opacity: 0;"></view></view>
		...
		<view class="message-wrapper" :style="{'margin-bottom': msgBoxBottom + 'px'}" ref="footer" id="footer">
			...
			<view class="message-line">
				...
				<textarea
					:key="'msgTextarea_' + textareaRefreshKey"
					v-show="!isSpeak && !isFace"
					id="msgTextarea" 
					ref="msgTextarea" 
					maxlength="512" 
					fixed="true"
				...
				<view
					v-show="!isSpeak && isFace"
					class="message-input message-input-preview"
					:style="{height: (msgHeight || 44)+'px'}"
					@click="showKeyboard()">
					<text>{{getMsgContentBeforeCursor()}}</text>
					<view class="fake-input-cursor"></view>
					<text>{{getMsgContentAfterCursor()}}</text>
				</view>
				...
				<view class="icons-wrapper" :style="{'align-self': alignSelf}">
					...
					<image src="../../static/icons/hr/msg-options.png" v-if="msgContent.length <= 0" class="icon-message" @click="showPlusBoard()"></image>
					<view v-else class="msg-send-btn" @click="sendMsg()">
						<text class="msg-send-text">发送</text>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="face-board-wrapper" :animation="animationData" style="height: 365px;">
			<scroll-view class="face-box" scroll-y="true">
				...
				<view class="latest-face" v-if="latestFaceList.length > 0">
					<block v-for="(face, index) in latestFaceList" :key="'latest_face_' + index">
						<image :src="getFaceSrc(face)" @click="enterFace(face)" class='ms-emoji'/>
					</block>
				</view>
				...
				<view class="emoji-wrapper">
					<block v-for="(face, index) in faceArray" :key="'face_' + index">
						<image :src="face.src" @click="enterFace(face.id)" class='ms-emoji'/>
			...
			<view class="face-operator-wrapper">
				<view class="btn-operator" @click="deleteWords">
					<image src="../../static/icons/icon-del-word1.png" class="icon-del-word"></image>
				</view>
			</view>
...
<script>
	...
	import { fluentEmojiList, getFluentEmojiById, isFluentEmojiId, replaceFluentEmojiWithImages } from "@/components/fluentEmoji.js";
	...
	export default {
		...
		data() {
			...
			const faceArray = fluentEmojiList;
		...
		// console.log("this.latestFaceList = " + this.latestFaceList);
			...
			if (latestFaceList != null && latestFaceList != undefined && latestFaceList != "") {
				latestFaceList = latestFaceList.filter(function(faceId) {
					return isFluentEmojiId(faceId);
				});
		...
		methods: {
			...
			getMsgList(page) {
				...
				uni.request({
					...
					header: {
						headerUserId: userId,
						headerUserToken: app.getUserSessionToken(),headerUserTokenKey: app.getUserSessionTokenKey()
					},
			...
			dealReceiveMsg(msgJSON) {
				...
				var myId = getApp().getUserInfoSession().id;
			...
			dealReceiveMsgSlef(msgJSON) {
				...
				console.log("friendId = " + friendId);
				// 如果消息发送者不是当前页面正在聊天的，则不处理
				if (senderId == myId && receiverId == this.friendId) {
					console.log("进来了~~~");
				} else {
					return;
				}
			...
			goDetail() {
				uni.navigateTo({
					url: "/pages/msgList/singleChatDetail",
			...
			saveLastestMsgToLocal(msgContent, chatTime) {
				var sourceUser = this.friendInfo;
				...
				for ( var i = 0 ; i < lastestUserChatList.length ; i ++) {
					...
					if (tmp.sourceUserId == lastMsg.sourceUserId) {
						lastestUserChatList.splice(i, 1);
						dealMsg = true;
						break;
					}
				}
			...
			transform2Words() {
				var postData = {
					...
					cuid: 'nlizzard', //用户唯一标识，用来区分用户，计算UV值。建议填写能区分用户的机器 MAC 地址或 IMEI 码，长度为60字符以内。
					token: this.token,
			...
			enterFace(faceId) {
				...
				var face = this.getFaceById(faceId);
				if (face == null) {
					return;
				}
				var cursor = this.cursor;
				var msgContent = this.msgContent;
				if (cursor == null || cursor == undefined || cursor < 0 || cursor > msgContent.length) {
					cursor = msgContent.length;
				}
				...
				var faceSign = face.emoji;
				this.msgContent = frontPart + faceSign + backPart;
				this.cursor = frontPart.length + faceSign.length;
				
				//把face存入本地list
				var latestFaceList = this.latestFaceList;
				// 判断，如果faceId已经存在，删除他，把新的放入
				latestFaceList = this.removeOldItem(latestFaceList, face.id);
				latestFaceList.unshift(face.id);	// 把点击的表情放在第一个，也就是最近使用的，作为数组开头元素
			...
			getFaceById(faceId) {
				if (faceId != null && faceId.id != undefined) {
					return faceId;
				}
				return getFluentEmojiById(faceId);
			},
			getFaceSrc(faceId) {
				var face = this.getFaceById(faceId);
				return face == null ? "" : face.src;
			},
			...
			deleteWords() {
				...
				if (faceBody != null && faceBody != undefined && faceBody != "") {
					var isFaceExist = isFluentEmojiId(faceBody);
				...
				if (isFaceExist) {
					...
					var leftText = msgContent.substring(0, cursor);
					var leftChars = Array.from(leftText);
					leftChars.pop();
					var leftContent = leftChars.join("");
					...
					// 删除后，光标前移1格
					this.cursor = leftContent.length;
				}
			},
			...
			sendMsg(e) {
				...
				// 消息时间（公用化）存储上一个消息时间，用于比对显示，如果本地时间没，则显示时间
				msgItem = this.dealMsgDateTime(msgItem);
			...
			replaceAndShowFace(msg) {
				return replaceFluentEmojiWithImages(msg, "emoji-iamge");
			},
			...
			**/  
			...
			typingMsg(e) {
				...
				// 	console.log(data);
				// }).exec();
				if (e != undefined && e != null && e.detail != undefined) {
			...
			getMsgContentBeforeCursor() {
				return this.msgContent.substring(0, this.cursor);
			},
			
			getMsgContentAfterCursor() {
				return this.msgContent.substring(this.cursor);
			},
